...
export default function Order() {
  ...
  const handleCheckout = () => {
    if (!user) {
      // 1. Capture the current intent
      localStorage.setItem("last_active_store_id", store.id);

      // 2. Redirect to signup/login
      navigate("/signup");
    } else {
      // 3. If they ARE logged in, proceed to the actual buying logic
      processOrder();
    }
  };

  const processOrder = () => {
    console.log("Pushing order to Firebase for store:", store.id);
    // This is where your payment or DB logic lives
  };
